...
TEST(PacketTest, CreateConnectionConfirmPacketGoodTest) {
  ...
  Packet packet =
      Packet::CreateConnectionConfirmPacket(1, 15, std::move(extraData))
          .value();
  EXPECT_TRUE(packet.IsControlPacket());
  EXPECT_EQ(packet.GetControlCommandNumber(),
            Packet::ControlPacketType::kControlConnectionConfirm);
  ASSERT_EQ(packet.GetBytes().size(), 8);
  auto expected = absl::StrFormat("%c%c%c%c%c%s", 0b10000001, 0x00, 0x01, 0x00,
                                  0x0F, kExtraData);
  EXPECT_EQ(expected, packet.GetBytes());
}
...
TEST(PacketTest, CreateConnectionRequestGoodPacketTest) {
  ByteArray extraData = ByteArray(std::string(kExtraData));
  Packet packet = Packet::CreateConnectionRequestPacket(0x0001, 0x0001, 0x000F,
                                                        std::move(extraData))
                      .value();
  EXPECT_TRUE(packet.IsControlPacket());
  EXPECT_EQ(packet.GetControlCommandNumber(),
  ...
  auto expected =
      absl::StrCat(absl::StrFormat("%c%c%c%c%c%c%c%s", 0b10000000, 0x00, 0x01,
                                   0x00, 0x01, 0x00, 0x0F, kExtraData));
  EXPECT_EQ(expected, packet.GetBytes());
}

TEST(PacketTest, CreateConnectionRequestBigProtocolVersionPacketTest) {
  ByteArray extraData = ByteArray(std::string(kExtraData));
  Packet packet = Packet::CreateConnectionRequestPacket(0x1001, 0x1001, 0x000F,
                                                        std::move(extraData))
                      .value();
  EXPECT_TRUE(packet.IsControlPacket());
  EXPECT_EQ(packet.GetControlCommandNumber(),
...
TEST(PacketTest, CreateConnectionRequestBadPacketTest) {
  EXPECT_FALSE(Packet::CreateConnectionRequestPacket(
