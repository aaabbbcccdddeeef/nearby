...
class PacketSequenceNumberGenerator {
 public:
  ...
  // Increments the counter if last_packet_counter is not 7.
  // Only used when remote control packet sequence numbers go out of sync with
  // the local counter.
  void Increment(int last_packet_counter);

  // Resets the counter to 0.
  void Reset();

 private:
  void IncrementLocked(int last_packet_counter)
