...
TEST(MessageWriteRequestTest, TestResetShortMessage) {
  ...
  EXPECT_FALSE(request.IsFinished());
  EXPECT_FALSE(request.IsStarted());
  EXPECT_EQ(request.NextPacket(15).value().GetPayload(), kShortMessage);
}
...
TEST(MessageWriteRequestTest, TestResetLongRequest) {
  ...
  EXPECT_EQ(nextPacket.GetPayload(), kLongFirstHalf);
  EXPECT_FALSE(request.IsFinished());
  ...
  EXPECT_EQ(lastPacket.GetPayload(), kLongSecondHalf);
  EXPECT_TRUE(request.IsFinished());
  request.Reset();
  EXPECT_FALSE(request.IsStarted());
  EXPECT_FALSE(request.IsFinished());
...
TEST(MessageWriteRequestTest, TestGetSetFuture) {
  MessageWriteRequest request = MessageWriteRequest(kShortMessage);
  std::future<absl::Status> result = request.GetFutureResult();
