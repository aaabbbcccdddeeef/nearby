...
#define ERROR_
const WCHAR PIE_HW_ID_[] = L"SWC\\VID_8086&PID_PIE&SID_0001\0";
const WCHAR PIE_DLL_PATH_HINT[] = L"PiePathHint";
}  // namespace

typedef MUROC_RET(APIENTRY* WIFIGETADAPTERLIST)(PINTEL_WIFI_HEADER pHeader,
                                                PVOID* pAdapterList);
...
typedef MUROC_RET(APIENTRY* FREELISTMEMORY)(LPVOID pList);

// Forward declarations of the internal private functions
PWSTR GetEntireRegistryDeviceList();
BOOL IsHwIdMatching(DEVINST devInst, const WCHAR* expecedHwId);
DEVINST SearchForDeviceInstance(PWSTR pEntireDeviceList);
...
DWORD GetRegKeyWCHARValue(DEVINST deviceInstance, LPCWSTR keyName,
                          WCHAR* valOut, PDWORD pValLen, PDWORD pDataType);
...
void FreeMemoryList(HINSTANCE murocApiDllHandle, LPVOID ptr);
...
uint8_t WifiIntel::GetGOChannel() {
  ...
  WifiPanQueryPreferredChannelSettingFunc =
      (WIFIPANQUERYPREFFEDCHANNELSETTING)GetProcAddress(
...
PWSTR GetEntireRegistryDeviceList() {
  ...
  if (configRet == CR_SUCCESS) {
    ...
    pDeviceList = (PWSTR) new BYTE[deviceListLength * sizeof(WCHAR)];
...
BOOL IsHwIdMatching(DEVINST devInst, const WCHAR* expecedHwId) {
  ...
  CONFIGRET configRet;
  WCHAR currentDeviceHwId[MAX_DEVICE_ID_LEN] = {0};
  ULONG propertySize;
  ...
  configRet = CM_Get_DevNode_PropertyW(devInst, &DEVPKEY_Device_HardwareIds,
                                       &propertyType, (PBYTE)currentDeviceHwId,
                                       &propertySize, 0);
  if (configRet == CR_SUCCESS) {
    // Compare to given HW ID
    WCHAR* pdest = wcsstr(currentDeviceHwId, expecedHwId);
    if (nullptr != pdest) {
...
VOID OpenRegKeyHandle(DEVINST devInst, HKEY& softwareKey) {
  ...
  if (devInst != NULL) {
    ...
    configRet =
        CM_Open_DevNode_Key(devInst, KEY_READ, 0, RegDisposition_OpenExisting,
...
DWORD GetFullDllLoadPathFromPieRegistry(DEVINST pieDeviceInstance,
  ...
  pLoadPathString = new WCHAR[dllFullPathLen];
  SecureZeroMemory(pLoadPathString, dllFullPathLen);
  ...
  wcscpy_s(pLoadPathString, dllFullPathLen, fullString.c_str());
...
HINSTANCE WifiIntel::PIEDllLoader() {
  PWSTR pEntireDeviceList = nullptr;
  ...
  if (ret == ERROR_SUCCESS) {
    ...
    murocApiDllHandle = LoadLibraryW(pDllPathValue);
...
HADAPTER WifiGetAdapterList(HINSTANCE murocApiDllHandle,
  ...
  INTEL_WIFI_HEADER intelHeader = {INTEL_STRUCT_VERSION_V156,
